            ...
            <div className="rounded-[2rem] bg-white p-8 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-[#555555] font-body">
                <div className="flex items-center gap-1">
                  {[1,2,3,4,5].map(i => (
                    <Star key={i} size={16} className="text-[#e63946]" />
                  ))}
                </div>
              ...
              <div className="mt-6 flex items-center gap-4">
                ...
                {product.original_price && (
                  <p className="text-sm text-[#888888] line-through font-body">₦{product.original_price.toLocaleString()}</p>
                )}
              </div>
              <p className="mt-6 text-[#555555] leading-relaxed font-body">
                {product.description || 'A premium statement piece designed for modern streetwear style.'}
              </p>
              ...
              <div className="mt-8 space-y-6">
                ...
                <div className="grid gap-4">
                  ...
                  <button 
                    ...
                    className="w-full rounded-full border border-[#111111] bg-white px-6 py-4 text-sm font-black uppercase tracking-[0.35em] text-[#111111] transition hover:bg-[#f5f5f5]"
                  >
                    Buy Now
                  </button>
                </div>
            ...
            {relatedProducts.map((item) => (
              <Link key={item.id} to={`/product/${item.id}`} className="group overflow-hidden rounded-[1.75rem] border border-[#eeeeee] bg-[#f9f9f9] transition hover:shadow-xl">
                <div className="relative overflow-hidden">
                  <img
                    src={item.images?.[0] || item.image_url || item.image || 'https://images.unsplash.com/photo-1483985988355-763728e1935b?w=400&h=500&fit=crop'}
                    alt={item.name}
                    loading="lazy"
                    onError={(e) => e.target.src = 'https://images.unsplash.com/photo-1483985988355-763728e1935b?w=400&h=500&fit=crop'}
                ...
                <div className="p-5">
                  ...
                  <p className="mt-3 text-lg font-black text-[#111111] font-body">₦{item.price?.toLocaleString() || '0'}</p>
                </div>
              </Link>
            ))}
          </div>

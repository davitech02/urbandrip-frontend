        ...
        try {
            ...
            const response = await fetch(
                `${API_URL}/api/orders/${selectedOrder.id}/status`,
                {
                    ...
                    body: JSON.stringify({
                        status: newStatus,
                        tracking_note: trackingNote || `Status updated to ${newStatus}`
                    })
                }
            );
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {filteredOrders.map((order) => (
                            <tr key={order.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 font-['Inter'] text-sm text-gray-900">{order.user?.full_name || order.customer_name}</td>
                    ...
                    <div className="bg-white rounded-xl max-w-md w-full">
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                <p className="font-['Inter'] text-sm text-gray-900">{selectedOrder.user?.full_name || selectedOrder.customer_name}</p>
                            </div>
                            ...
                            <div>
                                <label className="block font-['Inter'] text-sm font-medium mb-2">Tracking Note (optional)</label>
                                <textarea
                                    value={trackingNote}
                                    onChange={(e) => setTrackingNote(e.target.value)}
                                    placeholder="e.g. Package dispatched via DHL, tracking ID: ..."
                                    rows={2}

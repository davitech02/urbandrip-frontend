...
const AdminSettings = () => {
    ...
    const fetchSettings = async () => {
        try {
            ...
            // Backend returns a flat dict, not wrapped in 'settings'
            const data = res.data || {};
            setSettings(prev => ({
                ...prev,
                ...
                shipping_fee: data.shipping_fee ?? prev.shipping_fee,
    ...
    useEffect(() => {
        Promise.all([
            ...
            setDiscountCodes(codesRes.data.codes || []);
        }).catch(error => {
            console.error('Error fetching settings:', error);
            setDiscountCodes([]);
    ...
    const handleCodeSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                max_usage: codeForm.max_usage ? parseInt(codeForm.max_usage) : null
            };
            ...
            setCodeForm({ code: '', discount_percentage: '', max_usage: '', expiry_date: '' });
            fetchDiscountCodes();
        } catch (error) {
                    ...
                    <div className="bg-white rounded-xl max-w-md w-full">
                        ...
                        <form onSubmit={handleCodeSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    type="number"
                                    value={codeForm.max_usage}
                                    onChange={(e) => setCodeForm({ ...codeForm, max_usage: e.target.value })}
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg font-['Inter'] focus:outline-none focus:border-blue-500"
                                    required
                                />
                            </div>
